...
public class PullRequestViewModel extends AndroidViewModel {
    ...
    private PullRequestRepository mRepository;
    // Using LiveData and caching what getPullRequestsSorted returns has several benefits:
    // - We can put an observer on the data (instead of polling for changes) and only update the
    //   the UI when the data actually changes.
    // - Repository is completely separated from the UI through the ViewModel.
    private LiveData<List<R_PullRequest>> mAllPullRequests;

    public PullRequestViewModel(Application application) {
        super(application);
        mRepository = new PullRequestRepository(application);
        mAllPullRequests = mRepository.getAllWords();
    }
